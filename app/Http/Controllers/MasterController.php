...
class MasterController extends Controller
{
    ...
    public function bulkDeleteMakeYear(Request $request)
    {
        ...
        ProductTemplate::whereIn('generation_id', $ids)->update(['is_deleted' => 1]);
        Product::whereIn('generation_id', $ids)->update(['is_deleted' => 1]);
    ...
    // public function viewPolicy($lang_code ,$id=null)
    // {
    ...
    public function viewPolicy($lang_code, $id = null)
    {
        ...
        $langMap = [
            'EN' => 'policy_content',
            'AR' => 'policy_content_ar',
            'FR' => 'policy_content_fr',
            'RU' => 'policy_content_ru',
            'FA' => 'policy_content_fa',
            'UR' => 'policy_content_ur',
        ];

        $column = $langMap[$lang_code] ?? 'policy_content';
        $policy->translated_content = $policy->$column;

        return view('admin.policy_view', compact('policy', 'lang_code'));
    }
    ...
    public function import(Request $request)
    {
        ...
        try {
            ...
            $headers = array_map('strtolower', array_map('trim', $rows->first()->toArray()));
            ...
            foreach ($rows->skip(1) as $row) 
            {
               $row = array_combine($headers, $row->toArray());
                $cat_id    = $row['id'] ?? '';
                 ...
                 $subcatName = trim($row['part type en'] ?? '');
            $frName     = trim($row['part type fr'] ?? '');
            $ruName     = trim($row['part type ru'] ?? '');
            $faName     = trim($row['part type fa'] ?? '');
            $urName     = trim($row['part type ur'] ?? '');
    
    
                $data = [
                            'fr_category_name' => $frName,  // Excel: subcategory_fr
                            'ru_category_name' => $ruName,  // Excel: subcategory_ru
                            'fa_category_name' => $faName,  // Excel: subcategory_fa
                            'ur_category_name' => $urName,  // Excel: subcategory_ur
    ...
    //echo "<pre>";print_r($data);die();
                $updated = DB::table('category')
                            ...
                            ->update($data);
               if ($updated) {
                    \Log::info("UPDATE `category` SET `fr_category_name` = '$frName', `ru_category_name` = '$ruName', `fa_category_name` = '$faName', `ur_category_name` = '$urName' WHERE `category`.`id` = $cat_id;");
                } else {
                    \Log::warning("⚠️ No match found for: $cat_id (Category ID: $cat_id)");
                }
               
            }
    
            
            return back()->with('success', 'Subcategories imported/updated successfully.');
        } catch (\Exception $e) {
