...
class HomeController extends Controller
{
    public function index(Request $request)
    {
        ...
        // $brand = $brands->map(function ($item) use ($locale) {
        //     if (in_array($locale, ['ar', 'fa', 'ur'])) {
        //         // Arabic versions
        //         $item->brand_name = $item->ar_brand_name ?? $item->brand_name;
        //     } else {
        ...
        return view('web.index', compact('brand', 'model', 'category', 'subcategory' , 'make_year', 'country', 'city'));
    }
    ...
    //         if (in_array($locale, ['ar', 'fa', 'ur'])) {
    //             // Arabic versions
    //             $item->model_name = $item->ar_model_name ?? $item->model_name;
    //         } else {
    //             // English + French + Russian
    ...
    //             DB::raw(
    //                 in_array($locale, ['ar', 'fa', 'ur'])
    //                     ? 'shop_detail.shop_name_ar as shop_name'
    //                     : 'shop_detail.shop_name as shop_name'
    //             ),
    ...
    public function productList(Request $request)
    {
        ...
        $productQuery = Product::query()
            ...
            ->select(
                ...
                // DB::raw(
                //     in_array($locale, ['ar', 'fa', 'ur'])
                //         ? 'brand.ar_brand_name as brand_name'
                //         : 'brand.brand_name as brand_name',
                // ),
                // // 'make_model.model_name as model_name',
                // DB::raw(
                //     in_array($locale, ['ar', 'fa', 'ur'])
                //         ? 'make_model.ar_model_name as model_name'
                //         : 'make_model.model_name as model_name',
                // ),

                DB::raw(
                    in_array($locale, ['ar', 'fr', 'ru', 'fa', 'ur'])
                        ? "COALESCE(NULLIF(brand.{$locale}_brand_name, ''), brand.brand_name) as brand_name"
                        : "brand.brand_name as brand_name"
                ),
                DB::raw(
                    in_array($locale, ['ar', 'fr', 'ru', 'fa', 'ur'])
                        ? "COALESCE(NULLIF(make_model.{$locale}_model_name, ''), make_model.model_name) as model_name"
                        : "make_model.model_name as model_name"
                ),
                ...
                DB::raw(
                    in_array($locale, ['ar', 'fr', 'ru', 'fa', 'ur'])
                        ? "COALESCE(NULLIF(category.{$locale}_category_name, ''), '') as category_name"
                        : "category.category_name as category_name"
                ),
                //'subcategory.subcat_name as subcategory_name',
                DB::raw(
                    in_array($locale, ['ar', 'fr', 'ru', 'fa', 'ur'])
                        ? "COALESCE(NULLIF(subcategory.{$locale}_subcat_name, ''), '') as subcategory_name"
                        : "subcategory.subcat_name as subcategory_name"
                ),
                ...
                'shop_detail.shop_name as shop_name_en',

                'shop_detail.shop_logo',
                'master_city.city_name',
                'master_country.country_name',
                'master_country.currency',
                'generation_year.start_year',
                'generation_year.end_year',
                'users.mobile',
                'users.latitude as user_lat',
        ...
        // STEP 4: Search by group combinations first + direct matches
        if (!empty($groupCombinations)) {
            $productQuery->where(function ($q) use ($groupCombinations, $request, $filterYear) {
                // Match all group combinations
                foreach ($groupCombinations as $combo) {
                    $q->orWhere(function ($sub) use ($combo, $request, $filterYear) {
                        $sub->where('product.brand_id', $combo->brand_id)
                            ->where('product.model_id', $combo->model_id)
                            ->where('product.category_id', $combo->category_id)
                            ->where('product.subcategory_id', $combo->subcategory_id);

                        // Variant match (if set)
                        if (!empty($combo->variant_id)) {
                            $sub->where('product.part_type_id', $combo->variant_id);
                        }

                        // Year check
                        if ($filterYear && !empty($combo->generation_id)) {
                            $sub->where('product.generation_id', $combo->generation_id)
                                ->where('generation_year.start_year', '<=', $filterYear)
                                ->where('generation_year.end_year', '>=', $filterYear);
                        }
                ...
                // Also allow direct matches (in case not all products are in interchange)
                $q->orWhere(function ($direct) use ($request, $filterYear) {
                    if ($request->brand_id)       $direct->where('product.brand_id', $request->brand_id);
                    if ($request->model_id)       $direct->where('product.model_id', $request->model_id);
                    if ($request->category_id)    $direct->where('product.category_id', $request->category_id);
    ...
    public function productDetail(Request $request, $id)
    {
        ...
        // if (in_array($locale, ['ar', 'fa', 'ur'])) {
        //     // Always use Arabic columns
        //     $brandCol       = 'ar_brand_name';
        //     $modelCol       = 'ar_model_name';
        //     $categoryCol    = 'ar_category_name';
        //     $subcategoryCol = 'ar_subcat_name';
        ...
        //             'u.mobile',
        //             //'sd.shop_name',
        //             DB::raw($locale == 'ar' ? 'sd.shop_name_ar as shop_name' : 'sd.shop_name as shop_name'),
        //             'sd.shop_name as shop_name_en',
        //             'u.id as seller_id',
        //             'co.country_name',
        //             'pm.product_image',
        ...
        // if (in_array($locale, ['ar', 'fa', 'ur'])) {
        //     // Brand & Model: only for Arabic locales
        //     $brandCol = 'ar_brand_name';
        //     $modelCol = 'ar_model_name';
        // } else {
        //     $brandCol = 'brand_name';
        ...
        // Category & Subcategory: for all supported locales
        if (in_array($locale, ['ar', 'fr', 'ru', 'fa', 'ur'])) {
            ...
            $subcategoryCol = 'subcat_name';
            $brandCol = 'brand_name';
            $modelCol = 'model_name';
        }

        $descCol = match ($locale) {
            'ar' => "COALESCE(NULLIF(p.product_description_ar, ''), p.product_description)",
            'fr' => "COALESCE(NULLIF(p.product_description_fr, ''), p.product_description)",
            'ru' => "COALESCE(NULLIF(p.product_description_ru, ''), p.product_description)",
            'fa' => "COALESCE(NULLIF(p.product_description_fa, ''), p.product_description)",
            'ur' => "COALESCE(NULLIF(p.product_description_ur, ''), p.product_description)",
            default => "p.product_description",
        };
        ...
        $product = DB::table('product as p')
            ...
            ->select(
                ...
                'users.latitude as user_lat',
                'users.longitude as user_long',
                DB::raw("$descCol as product_description")
            )
            ->where('p.id', $id)
            ->first();
            ...
            if ($combinations->isNotEmpty()) {
                $similarProducts = DB::table('product as p')
                    ...
                    ->select(
                        ...
                        DB::raw('MAX(p.stock_number) as stock_number'),
                        DB::raw('MAX(p.product_price) as product_price'),
                        DB::raw('MAX(p.product_note) as product_note'),
                        DB::raw('MAX(pm.product_image) as product_image'),
                        DB::raw('MAX(b.brand_name) as brand_name'),
                        DB::raw('MAX(m.model_name) as model_name'),
                        DB::raw('MAX(c.category_name) as category_name'),
                        DB::raw('MAX(s.subcat_name) as subcat_name'),
                        DB::raw('MAX(u.first_name) as first_name'),
                        DB::raw('MAX(u.last_name) as last_name'),
                        DB::raw('MAX(u.latitude) as user_lat'),
                        DB::raw('MAX(u.longitude) as user_long'),
                        DB::raw('MAX(u.mobile) as mobile'),
                        DB::raw('MAX(sd.shop_name) as shop_name'),
                        DB::raw('MAX(sd.shop_name) as shop_name_en'),
                        DB::raw('MAX(u.id) as seller_id'),
                        DB::raw('MAX(co.country_name) as country_name'),
                        DB::raw('MAX(co.currency) as currency'),
                        DB::raw('MAX(gy.start_year) as start_year'),
                        DB::raw('MAX(gy.end_year) as end_year'),
                        DB::raw('MAX(py.part_type_label) as part_type_label')
                    )
        ...
        return view('web.product_detail', compact('product', 'user', 'shop', 'brand', 'model', 'category', 'subcategory', 'similarProducts', 'country', 'service', 'city'));
    }
    ...
    public function shopDetail(Request $request, $city, $shop)
    {
        ...
        $shop  = DB::table('shop_detail')
            ->select(
                ...
                DB::raw(
                    in_array($locale, ['ar', 'fr', 'ru', 'fa', 'ur'])
                        ? "COALESCE(NULLIF(shop_detail.about_shop_{$locale}, ''), shop_detail.about_shop) as about_shop"
                        : "shop_detail.about_shop as about_shop"
                ),
                ...
                'shop_detail.shop_logo as shop_banner',
                //'shop_detail.about_shop',
                'shop_detail.qr_code'
            )
...
*/
    ...
    public function directory_listing(){
        $shops = DB::table('users')
            ->join('shop_detail as shops', 'shops.user_id', '=', 'users.id')
            ->select('shops.id as shop_id','shop_name','shop_name_ar','city_id','country_id','address1','latitude','longitude')
            ->get();
        ...
        if(!empty($shops)){
            ...
            foreach($shops as $shop){
                ...
                $city_name = DB::table("master_city")->where('city_id',$shop->city_id)->first();
                $city_id = (!empty($city_name))?$city_name->city_name:'';
                $city_names = (!empty($city_name))?$city_name->city_id:'';
                $country_name = DB::table("master_country")->where('country_id',$shop->country_id)->first();
                $country_names = (!empty($country_name))?$country_name->country_id:'';
                $country_id = (!empty($country_name))?$country_name->country_name:'';

                $locale = App::getLocale();
                if($locale == 'ar' || $locale == 'ur' || $locale == 'fa'){
                    $shop_name = $shop->shop_name_ar;
                }else{
                    $shop_name = $shop->shop_name;
                }

                $shop_detail[] = array("id"=>$shop->shop_id,"name"=>$shop_name,"city"=>$city_names,"country"=>$country_names,"city_id"=>$city_id,"country_id"=>$country_id,"address"=>$shop->address1,"lat"=>(float) $shop->latitude,"lng"=>(float) $shop->longitude);
            }
        }
        $data['country_data'] = DB::table("master_country")->get();
        

        $data['shop_details'] = json_encode($shop_detail);
        // echo "<pre>";
        // print_r($data['shop_details']);die;
        return view("web.directory_listing")->with($data);
    }

    public function getCities(Request $request)
    {
        $countryIds = (array) $request->get('country_ids', []);

        $cities = \DB::table('master_city')
            ->whereIn('country_id', $countryIds)
            ->get(['city_id', 'city_name', 'country_id']);
