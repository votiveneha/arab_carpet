...
class HomeController extends Controller
{
    ...
    public function productDetail(Request $request, $id)
    {
        ...
        switch ($locale) {
            case 'ar':
                $descCol = "COALESCE(NULLIF(p.product_description_ar, ''), p.product_description)";
                break;
            case 'fr':
                $descCol = "COALESCE(NULLIF(p.product_description_fr, ''), p.product_description)";
                break;
            case 'ru':
                $descCol = "COALESCE(NULLIF(p.product_description_ru, ''), p.product_description)";
                break;
            case 'fa':
                $descCol = "COALESCE(NULLIF(p.product_description_fa, ''), p.product_description)";
                break;
            case 'ur':
                $descCol = "COALESCE(NULLIF(p.product_description_ur, ''), p.product_description)";
                break;
            default:
                $descCol = "p.product_description";
        }
                    ...
                    ->where(function ($query) use ($combinations) {
                        foreach ($combinations as $combo) {
                            $query->orWhere(function ($q) use ($combo) {
                                $q
                                    ->where('p.brand_id', $combo->brand_id)
...
*/
    ...
    public function directory_listing(){
        $shops = DB::table('users')
            ...
            ->select('shops.id as shop_id','shop_name','city_id','country_id','address1','latitude','longitude')
            ->get();
        ...
        if(!empty($shops)){
            ...
            foreach($shops as $shop){
                ...
                $shop_detail[] = array("id"=>$shop->shop_id,"name"=>$shop->shop_name,"city"=>$city_names,"country"=>$country_names,"city_id"=>$city_id,"country_id"=>$country_id,"address"=>$shop->address1,"lat"=>$shop->latitude,"lng"=>$shop->longitude);
            }
        }
