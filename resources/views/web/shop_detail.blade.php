@extends('web.common.layout') @section('content')
    ...
    <div class="shop-detail-add shop-product-detail shop-product-list-add">
        <div class="container mt-4">
            <div class="row shop-inner-detail-content shop-product-inner">
                <!-- Sidebar Filters -->
                <div class="col-md-3 filter-box shop-filters-left-side custom-select-wrapper in_desktop">
                            ...
                            @if ($model)
                                @foreach ($model as $models)
                                    <option value="{{ $models->id }}"
                                        {{ request('model_id') == $models->id ? 'selected' : '' }}>
    ...
    @endif
                        ...
                        <button class="btn btn-dark btn-block shop-search-btn">{{ __('messages.search') }}
                            {{ $shop->shop_name }} </button>
                ...
                <div class="col-md-6 shop-center-detail">
                    ...
                    @if ($shop->shop_banner)
                        <img src="{{ asset('/public/uploads/shop_image/' . $shop->shop_banner) }}" alt="Shop"
                            class="img-fluid mb-3" />
                    @else
                        <img src="{{ asset('/public/web_assets/images/shop-img.png') }}" alt="Shop"
                            class="img-fluid mb-3" />
                    @endif
                    <p>
                        {{ $shop->about_shop }}
                    ...
                    <div class="col-md-12 filter-box shop-filters-left-side custom-select-wrapper in_mobile">
                        <form method="get" action="{{ route('sellerMiniPage', ['id' => $user->id]) }}">
                            @csrf
                            <input type="hidden" name="user_latitude" id="user_latitude">
                            <input type="hidden" name="user_longitude" id="user_longitude">
    ...
    @endif
                    ...
                    {{-- Show socil icon in mobile --}}
                    <div class="contact-info mobSocilBox in_mobile">
                        <div class="info-text">
                            <div class="social-icons mt-2">
                                {{-- <a href="https://wa.me/{{ $user->mobile }}" target="_blank"><i
                                    class="bi bi-whatsapp"></i></a> --}}

                                <button class="s_icon" onclick="openWhatsApp('{{ $user->mobile }}', event)">
                                ...
                                <a class="s_icon" href="tel:{{ $phone }}"><i class="bi bi-telephone"></i></a>
                                {{-- <a href="#"><i class="bi bi-geo-alt-fill"></i></a> --}}

                                <button class="s_icon" style="border:none"
                                    onclick="openMap('{{ $user->user_lat }}','{{ $user->user_long }}', event)">
                ...
                <div class="col-md-3 contact-info">
                    ...
                    <div class="card bar-code-add" id="business_card"
                        style="width: 308px; background: white; padding: 20px; border-radius: 0px;">
                        ...
                        <div class="info">
                            ...
                            <div class="bar-code-img-add">
                                <img src="{{ asset('/public/uploads/qr_code/' . $shop->qr_code) }}" alt="Parts Rack"
                                    class="img-fluid mb-3" />
                    ...
                    <div class="info-text">
                        <p class="mb-1 font-weight-bold address-text-add">
                            {{ $user->address1 . ' , ' . $user->address2 . ' , ' . $user->zip_code . ' , ' . $user->city_name . ' , ' . $user->country_name }}
                        </p>

                        <p class="mb-1 phone-text">PHONE: <a href="tel:+{{ $user->mobile }}" class="phone-no-add"
                                target="_blank">{{ $user->mobile }}</a></p>
                        <p class="mb-1 phone-text">WHATSAPP: <a href="https://wa.me/{{ $user->mobile }}"
                                class="phone-no-add" target="_blank">{{ $user->mobile }}</a></p>
                        <div class="social-icons mt-2">
                            {{-- <a href="https://wa.me/{{ $user->mobile }}" target="_blank"><i
                                    class="bi bi-whatsapp in_desktop"></i></a> --}}


                            <button class="in_desktop s_icon" onclick="openWhatsApp('{{ $user->mobile }}', event)">
                                <i class="bi bi-whatsapp"></i>
                            </button>
                            @php
                                $phone = $user->mobile;
                            ...
                            <a class="in_desktop s_icon" href="tel:{{ $phone }}"><i
                                    class="bi bi-telephone"></i></a>
                            {{-- <a href="#"><i class="bi bi-geo-alt-fill"></i></a> --}}

                            <button class="in_desktop s_icon" style="border:none"
                                onclick="openMap('{{ $user->user_lat }}','{{ $user->user_long }}', event)">
                                <i class="bi bi-geo-alt"></i>
                            </button>
